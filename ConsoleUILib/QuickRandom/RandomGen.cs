...
namespace ConsoleUILib.QuickRandom
{
    public static class RandomGen
    {
        ...
        public static T RandomSelectOne<T>(List<T> list) where T : CanbeRandom
        {
        ...
        /// <returns>随机选中的项</returns>
        public static CanbeRandom RandomSelectOne<TItem>(List<CanbeRandom> list, double p)
            where TItem : CanbeRandom
        {
            if (list == null || list.Count == 0)
                throw new ArgumentException("列表不能为 null 或空");
            if (p < 0 || p > 1)
